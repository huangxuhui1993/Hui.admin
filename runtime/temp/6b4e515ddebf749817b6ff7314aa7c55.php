				...
				<ul class="cl">
					...
					<li id="Hui-msg"> 
						<a href="javascript:;" title="消息">
							<span class="badge badge-danger" id="message-count">0</span>
		...
		<dl id="menu-conf">
			...
			<dd>
				<ul>
					...
					<li><a data-href="<?php echo url('Logs/lis'); ?>" data-title="系统日志" href="javascript:void(0)">系统日志</a></li>
				</ul>
			</dd>
		</dl>
		<dl id="menu-dbmanage">
...
window.ajax_messages_url = "<?php echo url('Ajax/message'); ?>";
window.common_message_lis_url = "<?php echo url('Common/message_lis'); ?>";
$(function(){
	// 初始化页面
	initialize_page();
	$("body").Huitab({
	...
	// $.cookie('messageCount', 0, cookie_config); // 初始化消息数量cookie
	// $.cookie('messageIndexID', 0, cookie_config); // 初始化消息弹框唯一编号cookie
	// message(); // 初始化消息

	// 注册一个名为'MessageTimer'的定时器，每30秒运行一次
