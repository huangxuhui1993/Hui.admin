...
class HttpCurl{
     ...
     * @param  integer $followAction 是否允许被抓取的链接跳转
     * @param  integer $gzip         是否启用gzip压缩
     * @param  string  $format       格式
     * @return array
     */
    public static function get($url = '', $param = [], $header = [], $timeout = 3, $followAction = 0, $gzip = 0, $format = 'html'){
        $ch = curl_init();
        if(is_array($param)){
     ...
     * @param  integer $ssl    是否启用ssl
     * @param  string  $format 返回的格式
     * @return array
     */
    public static function post($url = '', $param = [], $header = [], $ssl = 0, $format = 'json'){
        $ch = curl_init();
        if(is_array($param)){
